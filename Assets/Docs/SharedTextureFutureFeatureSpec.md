...
- ただし `CustomRT` は `BaseShader/TextureSlot/CustomRT` を暫定使用しており、正規キー整理が未完了である
- `BaseShader.shader` / `Surface2D.hlsl` にはまだ `_ScreenFx*` と `ScreenFx2D.hlsl` が残っている
- つまり、外部 Texture の受け口はあるが、「受け取った外部 Texture をそのまま表示する専用機能」はまだない

#### Noise / Compute 側

- `MaterialFxMB` は非 WebGL では `ProceduralNoise2D.compute` を前提に `IKernelGenerator` と `MaterialFxKernelService` を登録している
- `AnimationSpriteChannelDef` / `AnimationSpriteChannelPlayer` も `KernelBindEnabled`, `InitialKernelTag`, `AtlasSlotIndex` を持ち、Runtime で Kernel slot を MaterialFx に流す経路を持っている
- `MaterialFxDispatchService` / `MaterialFxValueTypes` も `ComputeKernel` sender を前提にしている
- つまり、Noise はまだ SharedTexture 系とは独立した旧 Atlas / Kernel ラインに強く依存している
...
- `MaterialFxMB` における Compute 前提の初期化
...
- `Kernel2D.hlsl`
- `AnimationSpriteChannelDef.KernelBindEnabled`
- `AnimationSpriteChannelDef.InitialKernelTag`
- `AnimationSpriteChannelDef.AtlasSlotIndex`
- `AnimationSpriteChannelPlayer.InitializeKernelBinding()`
...
`AnimationSpriteChannelDef` にある Kernel bind 3 点セットは、将来的に次へ置き換える。

- `KernelBindEnabled` -> 廃止
- `InitialKernelTag` -> `SharedTextureTag` または binder 定義へ移行
- `AtlasSlotIndex` -> `SharedTextureBindSlot` へ移行
