...
- 範囲: ApplicationKernel / SceneKernel の 2 層 composition、Entity(Scope) 単位のサービス所有、AoS への移行方針、既存資産の保全、legacy 隔離、移行波の順序
- 非目標: v2 target kernel の意味論の再定義、別の kernel の新設、runtime discovery の温存、隠れた fallback の拡張

### 改訂メモ

この文書は v2.1 を「第二の kernel」として扱わない。
v2.1 は、v2 target kernel の意味論を前提にした live migration の実行層であり、`ApplicationKernel` と `SceneKernel` の 2 層を使って既存資産をまとめ直すための移行層である。
...
- v2.1 の移行目的と優先順位
- v2.1 の完了定義
- `ApplicationKernel` / `SceneKernel` の責務
...
v2.1 の完了は、代表的な service や特定 slice だけが新 path で動くことではない。
完了とは、既存の Scene / Prefab を少修正で維持したまま、コンパイルされたゲーム本体が `ApplicationKernel` / `SceneKernel` と新しい service / command / value / scalar / query / spawn runtime だけで起動・進行・終了する状態を指す。
したがって M12 完了時には、全 service / command surface の authority が新 path に移り、legacy bridge は runtime authority から外れていなければならない。
...
4. サービスの所有は `ApplicationKernel` と `SceneKernel` に分け、前者は game-wide / cross-scene、後者は scene-local / entity-scoped を持つ。
5. 1 つのサービス形で全てを統一しようとしない。
```

v2.1 は、旧 DI 型の利便性を一時的に受け継ぎつつ、最終的には `ApplicationKernel` が game-wide root を、`SceneKernel` が scene-local root を持つ明示的な所有へ移行する。

このため、v2.1 のサービスは次の 2 形に分類する。

ここでの 2 形は主に `SceneKernel` 配下の scene-local service を指す。
`ApplicationKernel` は別途 game-wide / cross-scene の shared service を所有できるが、それは scene-local entity-scoped service とは別の boundary に置く。

1. `Entity(Scope)` ごとに runtime instance を持つサービス。これは `SceneKernel` が所有する entity-scoped `ServiceGraph` の登録単位である。
2. `Entity(Scope)` ごとに AoS 形式の runtime data を持ち、data をまとめて処理するサービス

最初の段階では 1 を主軸とし、2 は将来の理想形として仕様化する。

この 2 形は target architecture を二重化する意図ではない。
v2.1 の現行実装は 1 を唯一の実装対象とし、2 は model-only の将来候補である。
2 を実装対象にする場合は、別途 AoS 専用の仕様と milestone を作ること。

ここでいう `SceneKernel` の entity-scoped `ServiceGraph` は、v2 target の coarse-grained `ServiceGraph` そのものではない。
v2.1 では migration-layer の ownership graph として使い、v2 core の ServiceGraph semantics は再定義しない。

2 層 kernel の composition ルール自体は [06_KernelLayerCompositionSpec.md](06_KernelLayerCompositionSpec.md) で固定する。
本書は、その composition を前提にした移行方針と preservation floor を定義する。
...
| command executor の大量登録を scope ごとに行っている | [CommandRunnerMB.cs](../../GameLib/Script/Common/Commands/MB/CommandRunnerMB.cs) | bulk registration が composition の主役になっている | command 系は entity-fixed `CommandRunner` と SceneKernel の明示的 catalog へ移し、旧登録は quarantine adapter とする |
...
`ApplicationKernel` と `SceneKernel` は、v2.1 で明示的に分離された 2 層の authority である。
`ApplicationKernel` は game-wide root、`SceneKernel` は scene-local root であり、両者を混ぜてはならない。

### `ApplicationKernel`

`ApplicationKernel` は DDOL に存在する game-wide root である。

`ApplicationKernel` の責務は次の通り。
...
`ApplicationKernel` は次のことをしてはならない。

- scene-local entity registration を直接持つこと
- `Transform.parent` を authority として扱うこと
- scene-local UI graph を直接所有すること
- `SceneKernel` の entity-scoped `ServiceGraph` を直接 mutation すること

### `SceneKernel`

`SceneKernel` は scene-local の移行制御点である。
これは scene-local authority であり、`ApplicationKernel` が選択済みの boot / profile / shared context を受けて、その scene に属する entity registration と service ownership を扱う。
v2.1 内部では、`SceneKernel` が entity-scoped `ServiceGraph` を所有し、`EntityRef` をキーに service を解決する。

`SceneKernel` の責務は次の通り。

- scene 内の service を明示的に所有する
- `EntityRef` をキーに service を登録・更新・破棄する
- entity-scoped `ServiceGraph` を構築し、resolve の唯一の入口とする
- UI 系 service については、service 内部の明示的な hierarchy / state graph を構築し、その入力を `SceneKernel` 経由で受ける
...
`ApplicationKernel` は game-wide のテーブル駆動の所有者であり、`SceneKernel` は scene-local のテーブル駆動の所有者である。
resolver 駆動の探索者であってはならない。
`EntityRef` を起点に `ServiceGraph` を引き、必要な service を明示的に解決する。
...
`EntityRef` は route key であり、lifetime token ではない。
current instance の有効性は lease handle / generation で検証する。
...
|---|---|---|---|---|
| 1. Entity(Scope) ごとの runtime instance | 1 Entity(Scope) に 1 service instance を持つ | `SceneKernel` の entity-scoped `ServiceGraph` が `EntityRef -> service instance` を保持する | current prefab / inspector surface を守りたいもの | phase 1 の主軸 |
...
`SceneKernel` が所有する entity-scoped `ServiceGraph` に explicit registration で組み立てる。
`EntityRef` で service を引けることが前提であり、型探索や hierarchy discovery に戻してはならない。
...
UI 系は、`ServiceGraph` に登録する前提を保ちながら、service 内部に explicit な hierarchy / state graph を持つ。
...
- `Entity` はすべて同じ枠組みで扱う
- UI の違いは、`ServiceGraph` に登録される service の構成で表す
- UI の論理階層は `Transform.parent` ではなく、service 内部の graph で所有する
- `Transform` は authoring / bridge の入力としては使ってよいが、runtime authority にしてはならない
- UI の状態遷移、selection、modal、navigation は explicit dependency と explicit graph で解決する
- UI の複雑な hierarchy は runtime discovery ではなく verified plan から構築する
- UI の hot path では hierarchy の broad search や毎フレームの再走査を行わない
- UI の lookup は cached handle / dense table / bounded traversal を前提にする
- UI の構造変化は毎フレームの allocation を生まない形に寄せる

UI 系は hierarchy を強く意識するが、その hierarchy は service-owned data structure であり、scene search の副産物ではない。
...
登録結果は `SceneKernel` 内部の entity-scoped `ServiceGraph` に反映される。
...
- `EntityRef + ServiceId` で resolve できるようにする
- per-entity DI scope を新しく増殖させない
- UI 系 service の内部 hierarchy は、service registration の時点で固定または検証済みでなければならない
- UI 系 service の hot path で hierarchy discovery や broad search を行ってはならない
- UI 系 service の resolve / navigation / selection は dense table か bounded lookup を使う
- UI 系 service は performance budget の範囲内で、必要な handle / node cache を維持する

`SceneKernel` は、service を discovery ではなく catalog で受け入れる。
entity-scoped `ServiceGraph` は、catalog から組み立てられた verified table として振る舞う。
...
### 1. `SceneKernel` が entity-scoped `ServiceGraph` を持つことと、v2 `ServiceGraph` の意味は同じではない
...
v2.1 の `ServiceGraph` は migration 層の entity-scoped graph として定義し、v2 core の coarse-grained graph とは別物として扱う。
...
- UI 系 service はここで優先的に扱い、`ServiceGraph` 登録と内部 hierarchy graph をセットで固める
...
- UI 系 service の hierarchy / state graph が explicit に定義されている
...
| テストケース | 目的 | 検証 |
|---|---|---|
| TC-V21-00-01 | `SceneKernel` が scene-local の service owner として明示されていることを確認する。 | `SceneKernel` が discovery ではなく entity-scoped `ServiceGraph` を受けると仕様に書かれていなければならない。 |
...
| TC-V21-00-03 | 形 1 が既存資産の preservation floor を守るための主軸であることを確認する。 | 既存 serialized field を保ったまま service registration を SceneKernel の entity-scoped `ServiceGraph` に移せると書かれていなければならない。 |
...
| TC-V21-00-07 | UI 系 service の hierarchy が explicit で、performance budget に従うことを確認する。 | UI の親子関係が `Transform` discovery ではなく service-owned graph と dense lookup で扱われると書かれていなければならない。 |
