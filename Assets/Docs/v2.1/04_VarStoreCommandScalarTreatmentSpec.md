...
- `CommandRunner` / command executor bootstrap / entity-fixed runtime placement の扱い
...
- `CommandRunnerMB` は bulk executor registration を持つ legacy authoring bridge である
- `CommandRunner` は execution engine と entity-fixed placement の境界がまだ明示されていない
...
### 8. Entity-scale runtime layout rule

entity 数が高密度になる target path では、`ValueStore` は per-entity の heavyweight service instance 群として作ってはならない。

target shape:

- `SceneKernel` または同等の manager-owned runtime が entity record 群を保持する
- entity-local value data は AoS 互換の compact record 群、または同等の scalable layout に載る
- per-entity object が必要でも、それは non-owning view / handle に留まり、truth storage を所有しない

残してはならないもの:

- entity ごとに `ValueStore` service class を常設生成すること
- entity ごとに MB を value truth owner として残すこと
- entity 数に比例して service registration / lifecycle object を無制限に増やすこと

v2.1 の移行途中で thin wrapper が一時的に残ることは許可する。
ただし、その wrapper は AoS 互換または同等の scalable layout へ移行できる形でなければならず、target completion の truth shape にはならない。
...
しかし `BlackboardMB` をそのまま target runtime installer や required runtime component にしてはならない。

entity が value / command / scalar を持つ場合でも、runtime authority は MB ではなく kernel-owned runtime data である。
...
`CommandRunnerMB` は target path に置かず、legacy authoring bridge に退ける。
...
- `CommandRunnerMB` による bulk registration host
...
phase 1 では、`CommandRunner` を 1 つの entity-fixed runtime component として持ってよい。
...
- `CommandRunnerMB` legacy authoring bridge
...
- `CommandRunner` が entity-fixed であり `CommandRunnerMB` が不要であると明記されている
...
| `TC-V21-04-03` | `CommandRunner` が entity-fixed で MB 不要であることを確認する | `CommandRunner` が Entity 固定で、`CommandRunnerMB` なしで command 実行できると書かれていなければならない |
