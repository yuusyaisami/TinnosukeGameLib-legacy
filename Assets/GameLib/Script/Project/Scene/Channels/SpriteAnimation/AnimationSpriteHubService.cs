...
namespace Game.Channel
{
    ...
    public sealed class AnimationSpriteHubService : IAnimationSpriteHubService, ITaggedMaterialFxProvider, IVisualHub, IScopeAcquireHandler, IScopeReleaseHandler, IScopeTickHandler, IDisposable
    {
            ...
            string hubTag = "default")
        {
            ...
            // Fallback: VContainer 縺ｮ optional parameter injection 縺瑚ｦｪ繧ｹ繧ｳ繝ｼ繝励°繧・
            // IMaterialFxPropertyRegistry 繧定ｧ｣豎ｺ縺ｧ縺阪↑縺・ｴ蜷医《cope resolver 縺ｧ蜀崎ｩｦ陦後☆繧・
            ...
            if (channelDefs != null)
            {
                ...
                {
                    // 蛻晄悄逋ｻ骭ｲ縺ｯ overwrite=false 縺ｧ謚募・
        ...
        {
            // ChannelDefBase 縺九ｉ閾ｪ蛻・・蝙九↓繧ｭ繝｣繧ｹ繝医＠縺ｦ蛻､螳・
        ...
        bool RegisterChannelInternal(AnimationSpriteChannelDef def, bool overwrite)
        {
            ...
            if (_players.ContainsKey(tag))
            {
                ...
                // 荳頑嶌縺崎ｨｱ蜿ｯ 竊・譌｢蟄倥メ繝｣繝阪Ν繧貞炎髯､縺励※縺九ｉ蜈･繧梧崛縺・
        ...
        bool RemoveChannelInternal(string tag)
        {
            ...
            // 荳蠢懈ｭ｢繧√※縺翫￥
            player.Stop();
            // Dispose 縺後≠繧後・遒ｺ螳溘↓遐ｴ譽・＠縺ｦ繝ｪ繝ｼ繧ｯ繧呈ｭ｢繧√ｋ
            try
            {
