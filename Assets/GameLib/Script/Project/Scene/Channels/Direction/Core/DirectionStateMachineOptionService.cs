...
namespace Game.Direction
{
        ...
        IScopeReleaseHandler
    {
        ...
        void EvaluateSingle(Vector2 value)
        {
            ...
            if (!hasH && !hasV)
            {
                // しきい値未満なら、方向を更新しない（Optionなしを作らないため）
                return;
            }
        ...
        void EvaluateDualAxis(Vector2 value)
        {
            ...
            // NOTE:
            // Movement.left / up などは親 OptionKey が同じでも、
            // full OptionValue キー（Movement.left, Movement.up）を併用して保持できるため
            // DualAxis では水平・垂直を同時に適用してよい。
        ...
        void EvaluateDualAxisCustom(Vector2 value)
        {
            ...
            if (!hasH && !hasV)
                return;
            // NOTE:
            // Custom mapping + DualAxis でも、同一親 OptionKey を理由に Single へ落とさない。
            // full OptionValue キーを使うことで重複方向（left+down 等）を保持できる。
