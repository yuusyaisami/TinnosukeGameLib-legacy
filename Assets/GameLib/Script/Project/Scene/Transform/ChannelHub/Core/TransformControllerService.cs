...
namespace Game.TransformSystem
{
    public sealed class TransformControllerService : IScopeTickHandler, IScopeAcquireHandler, IScopeReleaseHandler, IDisposable, ITransformTeleportService, ITransformAnimationOutputSink, ITransformChannelPoseReader
    {
        ...
        public void Tick()
        {
            ...
            if (_config.EnableMovement)
            {
                ...
                else
                {
                    ...
                    var suppressRigidbodyMovement = ConsumeRigidbodyMovementSuppressWindow(deltaTime);

                    if (!positionOverridden)
                    {
                        if (!_useBulkTransform)
                        {
                            if (!suppressRigidbodyMovement)
        ...
        bool ConsumeRigidbodyMovementSuppressWindow(float deltaTime)
        {
            if (_suppressRigidbodyMovementSeconds <= 0f)
                return false;
            ...
            _suppressRigidbodyMovementSeconds = Mathf.Max(0f, _suppressRigidbodyMovementSeconds - Mathf.Max(0f, deltaTime));
            return _suppressRigidbodyMovementSeconds > 0f;
        }
        ...
        public bool TryAddForceToRigidbody2D(Vector2 force, ForceMode2D mode = ForceMode2D.Force)
        {
            ...
            PrimeRigidbodyMovementSuppressWindow(mode);
            return true;
        }

        void PrimeRigidbodyMovementSuppressWindow(ForceMode2D mode)
        {
            if (!_config.EnableMovement)
                return;
            ...
            var suppressSeconds = mode == ForceMode2D.Impulse
                ? 0.12f
