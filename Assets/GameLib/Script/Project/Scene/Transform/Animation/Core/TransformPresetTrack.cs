...
namespace Game.TransformSystem
{
    ...
    public sealed class TransformPresetTrack : ITransformModifierTrack
    {
        ...
        bool _pivotActive;
        bool _scrollActive;
        bool _scrollUseLocalVelocity;
        bool _scrollUseAnchoredPosition;
        Vector3 _scrollVelocity;

        CancellationTokenSource? _cts;

        public bool IsAlive => !_stopped && (_playing || _activeTweenCount > 0 || _scrollActive);
            ...
            Vector3? overrideTo = null)
        {
            ...
            float duration = 0f;
            if (step.Operation != TransformAnimationOperation.Scroll)
            {
                duration = step.Duration.Get(_scope, variables, 0f);
                ...
                case TransformAnimationOperation.Scroll:
                    {
                        ...
                        var useAnchoredPosition = step.ScrollSpace == TransformScrollSpace.AnchoredPosition && rect != null;
                        var useLocalVelocity = !useAnchoredPosition && (step.ScrollUseLocalVelocity || step.Relative);

                        _scrollVelocity = velocity;
                        _scrollUseAnchoredPosition = useAnchoredPosition;
                        _scrollUseLocalVelocity = useLocalVelocity;
                        _scrollActive = velocity.sqrMagnitude > 1e-8f;
        ...
        {
            // Scroll は duration を使わず、Tick で速度積分して継続移動する。
            if (_stopped || !_scrollActive || deltaTime <= 0f)
                return;

            if (_scrollUseAnchoredPosition)
            {
                if (!_rectTransform)
                {
                    _scrollActive = false;
                ...
                _anchoredPosition += new Vector2(_scrollVelocity.x, _scrollVelocity.y) * deltaTime;
                return;
            }
            ...
            var worldVelocity = _scrollUseLocalVelocity
                ? _ownerTransform.TransformVector(_scrollVelocity)
                : _scrollVelocity;
            _worldPosition += worldVelocity * deltaTime;
        }
        ...
        void ClearActiveFlags()
        {
            ...
            _pivotActive = false;
            _scrollActive = false;
            _scrollUseLocalVelocity = false;
            _scrollUseAnchoredPosition = false;
            _scrollVelocity = Vector3.zero;
