...
namespace Game.UI
{
    ...
    public sealed class UISelectionMB : MonoBehaviour, IFeatureInstaller
    {
        ...
        {
            builder.Register<UINodeGraphService>(RuntimeLifetime.Singleton)
                .As<IUINodeGraphService>()
                .As<IUINodeGraphTelemetry>();
            ...
            builder.RegisterBuildCallback(container =>
            {
                ...
                if (container.TryResolve<IUISelectionService>(out var selectionService) &&
                    selectionService is UISelectionService concreteSelection &&
                    container.TryResolve<IUINodeGraphService>(out var nodeGraph))
                {
                    concreteSelection.SetNodeGraph(nodeGraph);
                }
            });
        }
    ...
    public sealed class UISelectionDeclarationMB : EntityDeclarationMB, IEntityServiceDeclarationAuthoring
    {
        [Header("Selection Service")]
        [LabelText("Selection Service Id")]
        [SerializeField]
        int selectionServiceId;

        [Header("Modal Dependency")]
        [LabelText("Modal Stack Service Id")]
        [SerializeField]
            ...
            out string failureReason)
        {
            if (selectionServiceId <= 0)
            {
                ...
                failureReason = "UISelectionDeclarationMB requires a positive selection service id.";
                return false;
            }
            ...
            {
                declarations = Array.Empty<EntityServiceDeclarationInput>();
                failureReason = "UISelectionDeclarationMB requires a positive modal stack dependency service id.";
                return false;
            }

            if (selectionServiceId == modalStackChannelHubServiceId)
            {
                declarations = Array.Empty<EntityServiceDeclarationInput>();
                failureReason = "UISelectionDeclarationMB requires distinct service ids for selection and modal stack dependencies.";
                return false;
            }
            ...
            declarations = new[]
            {
                new EntityServiceDeclarationInput(
                    ...
                    new ServiceId(selectionServiceId),
                    CreateStableId(declarationInput.OwnerEntityRef, selectionServiceId),
                    typeof(UISelectionService).Name,
                    typeof(UISelectionService).Name,
                    new[]
                    {
                        typeof(IUISelectionService).FullName ?? nameof(IUISelectionService),
                        typeof(IUISelectionState).FullName ?? nameof(IUISelectionState),
                        typeof(IUISelectionNavigation).FullName ?? nameof(IUISelectionNavigation),
                        typeof(IUISelectionTelemetry).FullName ?? nameof(IUISelectionTelemetry),
                        typeof(IUISelectionBlockService).FullName ?? nameof(IUISelectionBlockService),
                    },
                    new[]
                    {
                        new EntityServiceDependencyInput(new DependencyNodeIR(new ServiceId(modalStackChannelHubServiceId)), DependencyStrength.Required),
                    },
        ...
        protected override void OnValidate()
        {
            ...
            selectionServiceId = Mathf.Max(0, selectionServiceId);
        ...
        {
            if (Application.isPlaying)
                throw new InvalidOperationException("UISelectionDeclarationMB authoring state may only be mutated in edit mode.");
            ...
            if (selectionServiceId <= 0 && HasSourceLocation)
                selectionServiceId = ComputeMigratedServiceId(CreateSourceLocation(), 1_100_000_000);
        }

        public void SetServiceIds(int newSelectionServiceId, int newModalStackChannelHubServiceId)
        {
            if (Application.isPlaying)
                throw new InvalidOperationException("UISelectionDeclarationMB authoring state may only be mutated in edit mode.");

            selectionServiceId = Math.Max(0, newSelectionServiceId);
            modalStackChannelHubServiceId = Math.Max(0, newModalStackChannelHubServiceId);
        }
#endif

        static string CreateStableId(Game.Kernel.Abstractions.EntityRef ownerEntityRef, int serviceId)
        {
            return "entity-service:" + ownerEntityRef.Value + ":ui-selection:" + serviceId.ToString("D10");
        }
