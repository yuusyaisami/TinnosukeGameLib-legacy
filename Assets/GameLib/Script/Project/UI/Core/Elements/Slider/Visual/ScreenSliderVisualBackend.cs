...
namespace Game.UI
{
    internal sealed class ScreenSliderVisualBackend : ISliderVisualBackend
    {
        ...
        SliderScreenRangeSnapshot _lastRangeSnapshot;
        ...
        SliderScreenRuntimeInstance? _backgroundRuntime;
        SliderScreenRuntimeInstance? _handleRuntime;
        readonly List<SliderScreenRuntimeInstance> _segmentBars = new();
        readonly List<SliderScreenRuntimeInstance> _markers = new();

        bool _loggedBackgroundVisualMissing;
        bool _loggedSegmentBarVisualMissing;

        public ScreenSliderVisualBackend(
            IScopeNode owner,
        ...
        public void OnAcquire(IScopeNode scope, bool isReset)
        {
            ...
            _loggedBackgroundVisualMissing = false;
            _loggedSegmentBarVisualMissing = false;
        ...
        public void Tick()
        {
            ...
            var rangeStatus = SliderRuntimeHelpers.TryResolveScreenRangeSnapshot(
                _activeScope,
                _options,
                _canvas,
            ...
            var rangeChanged = !_hasLastRangeSnapshot || !SliderRuntimeHelpers.ApproximatelyEquals(_lastRangeSnapshot.LocalRect, rangeSnapshot.LocalRect);
        ...
        {
            SliderScreenRuntimeInstance? background = null;
            SliderScreenRuntimeInstance? handle = null;
            var bars = new List<SliderScreenRuntimeInstance>();
            var markers = new List<SliderScreenRuntimeInstance>();
            ...
            try
            {
                ...
                if (_visualizerPreset.Background.Enabled)
                {
                    ...
                    if (template != null)
                    {
                        background = await SpawnSizedInstanceAsync(
                            ...
                            "[SliderVisualizerService] Screen background requires a RectTransform/Image visual target.",
                            ct);
                    }
                }
                    ...
                    if (template != null)
                    {
                        for (var i = 0; i < Mathf.Max(0, _output.BoundaryCount - 1); i++)
                        {
                            ...
                            var instance = await SpawnSizedInstanceAsync(
                                ...
                                "[SliderVisualizerService] Screen segment bar requires a RectTransform/Image visual target.",
                                ct);
                            if (instance != null)
                                bars.Add(instance);
                        }
                ...
                Tick();
                if (background != null)
                    await ExecuteSpawnCommandsAsync(background.Runtime, _visualizerPreset.Background.OnSpawnCommands, ct);
                for (var i = 0; i < bars.Count; i++)
                    await ExecuteSpawnCommandsAsync(bars[i].Runtime, _visualizerPreset.Segmented.OnSegmentBarSpawnCommands, ct);
                for (var i = 0; i < markers.Count; i++)
                    await ExecuteSpawnCommandsAsync(markers[i].Runtime, _visualizerPreset.Segmented.OnMarkerSpawnCommands, ct);
                if (handle != null)
                    await ExecuteSpawnCommandsAsync(handle.Runtime, _visualizerPreset.Handle.OnSpawnCommands, ct);
            }
        ...
        void ApplySnapshot(in SliderOutputSnapshot snapshot, in SliderScreenRangeSnapshot rangeSnapshot)
        {
        ...
        {
            var center = SliderRuntimeHelpers.ResolveRectMarkerPosition(
                rangeSnapshot.LocalRect,
                _visualizerPreset.Segmented.FillAxis,
                _visualizerPreset.Segmented.OriginSide,
                normalizedValue);
        ...
        async UniTask<SliderScreenRuntimeInstance?> SpawnSizedInstanceAsync(
        ...
        async UniTask<SliderScreenRuntimeInstance?> SpawnTransformInstanceAsync(
            IAsyncSpawnerService spawner,
            BaseRuntimeTemplateSO template,
            Transform parent,
            SliderSpawnUnitKind unitKind,
        ...
        {
            if (_backgroundRuntime?.RootRect != null)
                _backgroundRuntime.RootRect.gameObject.SetActive(isActive);
            if (_handleRuntime?.RootRect != null)
                _handleRuntime.RootRect.gameObject.SetActive(isActive);

            for (var i = 0; i < _segmentBars.Count; i++)
            {
                if (_segmentBars[i].RootRect != null)
                    _segmentBars[i].RootRect.gameObject.SetActive(isActive);
            }

            for (var i = 0; i < _markers.Count; i++)
            {
                if (_markers[i].RootRect != null)
                    _markers[i].RootRect.gameObject.SetActive(isActive);
            }
        }
        ...
        void ReleaseRuntimeInstances(List<SliderScreenRuntimeInstance> instances)
        {
            for (var i = 0; i < instances.Count; i++)
                ReleaseRuntimeInstance(instances[i]);
        }

        void ReleaseRuntimeInstance(SliderScreenRuntimeInstance? instance)
        {
            if (instance == null)
                return;

            instance.RootState.ApplyTo(instance.RootRect);
            instance.SizeState.ApplyTo(instance.SizeRect);
            SliderRuntimeHelpers.RestoreSpawnedRuntime(instance.Runtime);
            SliderRuntimeHelpers.ReleaseSpawnedRuntime(instance.Runtime.Resolver);
        }
