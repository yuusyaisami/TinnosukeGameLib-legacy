...
namespace Game.Movement
{
        ...
        ITickable
    {
        ...
        readonly IObjectResolver _resolver = null!;
            ...
            IScopeNode scopeNode)
        {
            ...
            // v0.2: Homing/Motion は遅延解決。
            // Runtime 側では未登録サービス解決時にログ/例外ノイズが出るため、
            // 必要になるまで Resolve しない。
            _homing = null;
            _motion = null;
        ...
        public void Tick()
        {
            ...
            // Motion/Homing は時間依存のため、入力が継続している間は毎フレーム再計算する。
            // (例: GravityPull の AdditiveVelocity 積分)
