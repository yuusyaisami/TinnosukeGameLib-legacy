...
namespace Game
{
    ...
    {
        UniTask<KernelScopeHost> AcquireAsync(
        ...
        void Release(KernelScopeHost scope);
        bool TryEnqueueOnNextAcquire(KernelScopeHost scope, CommandListData commands, CommandRunOptions options);
    }
    ...
    public sealed class RuntimeLifetimeScopePool : IRuntimeLifetimeScopePool, IRuntimeLifetimeScopePoolTelemetry
    {
        ...
        sealed class ScopeRefComparer : IEqualityComparer<KernelScopeHost>
        {
            public static readonly ScopeRefComparer Instance = new();

            public bool Equals(KernelScopeHost? x, KernelScopeHost? y) => ReferenceEquals(x, y);
            public int GetHashCode(KernelScopeHost obj) => RuntimeHelpers.GetHashCode(obj);
        }
        ...
        readonly Dictionary<RuntimeLifetimeScopePoolKey, ObjectPool<KernelScopeHost>> _pools =
            new(new RuntimeLifetimeScopePoolKeyComparer());
        readonly Dictionary<KernelScopeHost, List<PendingOnAcquireCommand>> _pendingOnAcquireCommands =
            new(ScopeRefComparer.Instance);
        readonly HashSet<KernelScopeHost> _releasingScopes =
            new(ScopeRefComparer.Instance);
        readonly Dictionary<RuntimeLifetimeScopePoolKey, PoolTelemetryBucket> _telemetryBuckets =
            new(new RuntimeLifetimeScopePoolKeyComparer());
        readonly Dictionary<KernelScopeHost, ScopeLeaseTelemetry> _activeScopeTelemetry =
            new(ScopeRefComparer.Instance);
        ...
        public async UniTask<KernelScopeHost> AcquireAsync(
            BaseRuntimeTemplateSO template,
            Transform parent,
            Vector3 position,
            Quaternion rotation,
            ...
            CancellationToken ct = default)
        {
            ...
            if (!template.UsePooling)
            {
                ...
                ConfigureOnAcquire(template, scopeNonPooled, parent, position, rotation, lifetimeScopeParent);
            ...
            ConfigureOnAcquire(template, scope, parent, position, rotation, lifetimeScopeParent);
        ...
        public UniTask WarmupAsync(BaseRuntimeTemplateSO template, int count, CancellationToken ct = default)
        {
            ...
            var tmp = ListPool<KernelScopeHost>.Get();
            try
            {
            ...
            finally
            {
                ...
                ListPool<KernelScopeHost>.Release(tmp);
            }

            return UniTask.CompletedTask;
        }

        public void Release(KernelScopeHost scope)
        {
            if (scope == null || !scope)
                return;
            ...
            if (template == null)
            {
                VerifiedCompositionRuntime.ReleaseRuntimeScope(scope);
        ...
        public bool TryEnqueueOnNextAcquire(KernelScopeHost scope, CommandListData commands, CommandRunOptions options)
        {
            if (scope == null || !scope)
                return false;
        ...
        void CleanupDestroyedActiveScopes()
        {
            ...
            List<KernelScopeHost>? destroyedScopes = null;
            foreach (var pair in _activeScopeTelemetry)
            {
                ...
                destroyedScopes ??= ListPool<KernelScopeHost>.Get();
            ...
            ListPool<KernelScopeHost>.Release(destroyedScopes);
            TouchTelemetry();
        }
        ...
        static string GetScopeName(KernelScopeHost? scope)
        {
            if (scope == null || !scope)
                return "Destroyed";
        ...
        bool TryBeginRelease(KernelScopeHost scope)
        {
            if (scope == null)
                return false;
        ...
        void EndRelease(KernelScopeHost scope)
        {
            if (scope == null)
                return;
        ...
        async UniTask ExecutePendingOnAcquireAsync(KernelScopeHost scope, CancellationToken ct)
        {
            if (scope == null || !scope)
                return;
        ...
        void ClearPendingOnAcquire(KernelScopeHost scope)
        {
            if (scope == null)
                return;
        ...
        static bool ShouldDestroyInsteadOfReturnToPool(KernelScopeHost scope)
        {
            if (scope == null || !scope)
                return true;
        ...
        ObjectPool<KernelScopeHost> GetOrCreatePool(RuntimeLifetimeScopePoolKey poolKey)
        {
            if (_pools.TryGetValue(poolKey, out var pool))
                return pool;

            KernelScopeHost Create()
            {
                var prefab = poolKey.PrefabKey;
                var go = UnityEngine.Object.Instantiate(prefab);
                go.name = prefab.name;

                var scope = go.GetComponent<KernelScopeHost>();
                if (scope == null)
            ...
            void OnGet(KernelScopeHost scope)
            {
                if (scope == null)
                    return;
            ...
            void OnRelease(KernelScopeHost scope)
            {
                if (scope == null)
                    return;
            ...
            pool = new ObjectPool<KernelScopeHost>(
                createFunc: Create,
                actionOnGet: OnGet,
        ...
        KernelScopeHost CreateInstance(BaseRuntimeTemplateSO template)
        {
        ...
        KernelScopeHost CreatePooledInstance(GameObject prefab)
        {
            var go = UnityEngine.Object.Instantiate(prefab);
            go.name = prefab.name;
        ...
        void ConfigureOnAcquire(BaseRuntimeTemplateSO template, KernelScopeHost scope, Transform parent, Vector3 position, Quaternion rotation, IScopeNode? lifetimeScopeParent)
        {
            ...
            IScopeNode? resolvedBuildParent;
            if (lifetimeScopeParent != null)
            {
                resolvedBuildParent = lifetimeScopeParent;
            }
            else if (useVerifiedComposition)
            {
                resolvedBuildParent = _buildParent;
            }
            else
            {
                var runtimeParent = parent.GetComponentInParent<KernelScopeHost>(includeInactive: true);
                if (runtimeParent == scope)
                    runtimeParent = null;

                resolvedBuildParent = runtimeParent ?? _buildParent;
            }

            scope.SetExplicitBuildParent(resolvedBuildParent);

            if (useVerifiedComposition)
                RequireVerifiedRuntimeBinding(template, scope, resolvedBuildParent);
            else
                VerifiedCompositionRuntime.TryBindRuntimeScope(template, scope, resolvedBuildParent);

            scope.gameObject.SetActive(true);
        }

        static void RequireVerifiedRuntimeBinding(BaseRuntimeTemplateSO template, KernelScopeHost scope, IScopeNode? explicitParent)
        {
            ...
            if (VerifiedCompositionRuntime.TryBindRuntimeScope(template, scope, explicitParent))
                return;

            string parentDescription = explicitParent is Component component
                ? component.gameObject.name
                : explicitParent?.GetType().Name ?? "<root>";
