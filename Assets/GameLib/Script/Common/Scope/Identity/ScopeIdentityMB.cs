// Assets/Game/Script/Core/Identity/ScopeIdentityMB.cs
using UnityEngine;
...
namespace Game
{
    public interface IScopeIdentityService
    {
    ...
    [ExecuteAlways]
    public sealed class ScopeIdentityMB : MonoBehaviour, IScopeInstaller, IHaveLifetimeScopeKind
    {
        ...
        public static LifetimeScopeKind PredictKindFromComponent(Component component, LifetimeScopeKind current = LifetimeScopeKind.None)
        {
            if (component == null)
                return current;

            if (component.TryGetComponent<ScopeIdentityMB>(out var identity) && identity != null && identity.kind != LifetimeScopeKind.None)
                current = identity.kind;

            if (component is IHaveLifetimeScopeKind typed && typed.Kind != LifetimeScopeKind.None)
                current = current == LifetimeScopeKind.None ? typed.Kind : current;

            if (current != LifetimeScopeKind.None && current != LifetimeScopeKind.Runtime)
                return current;

            return PredictKindFromType(component.GetType(), current);
        }
        ...
        public static LifetimeScopeKind PredictKindFromType(Type t, LifetimeScopeKind current = LifetimeScopeKind.None)
        {
            ...
            if (typeof(KernelScopeHost).IsAssignableFrom(t))
        ...
        LifetimeScopeKind GuessKind()
        {
            ...
            // KernelScopeHost 縺ｯ LifetimeScope 繧堤ｶ呎価縺励↑縺・◆繧√∝句挨縺ｫ謗｢邏｢縺吶ｋ縲・
            var current = transform;
            while (current != null)
            {
                if (current.TryGetComponent<KernelScopeHost>(out var runtimeScope) && runtimeScope != null)
                {
                    return PredictKindFromComponent(runtimeScope, kind);
                }
        ...
        public void InstallScopeServices(IRuntimeContainerBuilder builder, IScopeNode owner)
        {
            ...
            // that represents the scope (prevents multiple child ScopeIdentityMB components from
            ...
            {
                Debug.LogWarning($"{nameof(ScopeIdentityMB)} on GameObject '{gameObject.name}' is not on the same GameObject as its owning LifetimeScope; it will not be registered into the container.", this);
            }

            // KernelScopeHost owns and registers its own RuntimeScopeIdentityService during ConfigureCore,
            // so avoid registering an IScopeIdentityService instance here for runtime scopes.
            if (owner is not KernelScopeHost)
            {
                builder.Register<ScopeIdentityService>(RuntimeLifetime.Singleton)
