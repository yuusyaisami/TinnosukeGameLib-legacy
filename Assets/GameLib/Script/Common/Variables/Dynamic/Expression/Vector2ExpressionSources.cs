// Game.Common.Vector2ExpressionSources.cs
//
// DynamicValue<Vector2> expression sources.
// - Vector2ExpressionSource: single expression expected to evaluate to Vector2.
// - Vector2XYExpressionSource: separate expressions for X and Y with shared variables.
...
namespace Game.Common
{
    ...
    public sealed class Vector2ExpressionSource : IDynamicSource, IExpressionSource, IExternalExpressionVariablesReceiver
    {
        ...
        public DynamicVariant Evaluate(IDynamicContext context)
        {
            ...
            try
            {
                ...
                if (!ExpressionHelper.TryAsVector2(result, out var vec2))
                    throw new InvalidOperationException("Expression result is not Vector2-compatible.");

                return DynamicVariant.FromVector2(vec2);
            }
            catch (Exception ex)
            {
        ...
        bool BuildCaches(out string error)
        {
            ...
            bool AddVars(IReadOnlyList<ExpressionVariable> vars, string label, bool allowOverrideExisting, out string localError)
            {
                ...
                for (int i = 0; i < vars.Count; i++)
                {
                    ...
                    var expectedKind = ResolveExpectedKind(v, fallback: ValueKind.Vector2);
    ...
    public sealed class Vector2XYExpressionSource : IDynamicSource, IExpressionSource, IExternalExpressionVariablesReceiver
    {
        ...
        public DynamicVariant Evaluate(IDynamicContext context)
        {
            if (_dirty || _compiledX == null || _compiledY == null)
            {
                if (!TryCompile(out _validationMessage))
                {
                    _validationIsError = true;
            ...
            if (_compiledX == null || _compiledY == null)
                return DynamicVariant.FromVector2(Vector2.zero);
        ...
        bool TryCompile(out string message)
        {
            ...
            if (string.IsNullOrWhiteSpace(_expressionX) || string.IsNullOrWhiteSpace(_expressionY))
            {
                message = "Expression X or Y is empty";
                _validationIsError = true;
                _compiledX = null;
                _compiledY = null;
                return false;
            ...
            try
            {
                ...
                if (!TryParse(_expressionX, "X", usedIdentifiers, out _compiledX, out message))
                {
                    _validationIsError = true;
                    _compiledY = null;
                    return false;
                ...
                if (!TryParse(_expressionY, "Y", usedIdentifiers, out _compiledY, out message))
                {
                    _validationIsError = true;
                    _compiledX = null;
                    return false;
        ...
        bool TryParse(string expression, string axisName, HashSet<string> usedIdentifiers, out ExpressionNode node, out string message)
        {
            node = null;
            message = null;
